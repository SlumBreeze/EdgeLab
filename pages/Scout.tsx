...
// Reference lines are now handled via useGameContext to ensure they are synced to Supabase
...
export default function Scout() {
  ...
  const { addToQueue, queue, referenceLines, setReferenceLine, scanResults, setScanResult } = useGameContext();

  const getOrUpdateReferenceLines = (gameId: string, pinnLines: BookLines | null) => {
    if (!pinnLines) return null;
    const existing = referenceLines[gameId];
    if (existing) return existing;

    const ref = {
      spreadLineA: pinnLines.spreadLineA,
      spreadLineB: pinnLines.spreadLineB
    };
    // Save to context (which syncs to Supabase)
    setReferenceLine(gameId, ref);
    return ref;
  };

  // Scan results are now handled via useGameContext to ensure they are synced to Supabase
  ...
  const handleLoadSlates = async () => {
    ...
    try {
      ...
      // Capture "Initial Pull" for the Day
      // This saves the first reference lines we see to Supabase for the whole day
      Object.values(allData).forEach((games: any) => {
        games.forEach((apiGame: any) => {
          const pinn = getBookmakerLines(apiGame, 'pinnacle');
          if (pinn && !referenceLines[apiGame.id]) {
            setReferenceLine(apiGame.id, {
  ...
  const handleQuickScan = async (game: Game) => {
    ...
    setScanResult(game.id, { signal: result.signal, description: result.description });
  ...
  const handleScanAll = async () => {
    ...
    for (const apiGame of gamesToScan) {
      ...
      try {
        ...
        setScanResult(apiGame.id, { signal: result.signal, description: result.description });
      } catch (e) {
        console.error(e);
      }
          ...
          <div className="flex overflow-x-auto space-x-2 pb-2 no-scrollbar">
            {Object.entries(SPORTS_CONFIG).map(([key, config]) => (
              <button
                ...
                className={`flex items-center space-x-2 px-4 py-2 rounded-full whitespace-nowrap transition-all shadow-sm ${selectedSport === key
        ...
        <div className="mb-6 flex gap-2">
          ...
          {apiGames.length > 0 && (
            <button
              ...
              className={`px-4 rounded-xl font-bold text-sm shadow-sm transition-all whitespace-nowrap ${batchScanning
      ...
      <div className="space-y-3">
        {!slatesLoaded ? (
          ...
          sortedGames.map(game => {
            ...
            const refLines = referenceLines[game.id];
            ...
            return (
              <div key={game.id} className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
                ...
                {scan && (
                  <div className={`absolute top-0 left-0 bottom-0 w-1.5 ${scan.signal === 'RED' ? 'bg-red-500' :
                ...
                <div className="flex justify-between items-center mb-2 pl-2">
                  ...
                  <button
                    ...
                    disabled={inQueue}
                    className={`px-2 py-1 rounded text-[10px] font-bold transition-colors ${inQueue
                      ? 'bg-slate-100 text-slate-400'
                ...
                <div className="pl-2">
                  {scan ? (
                    <div className={`p-2 rounded-lg flex items-start gap-2 ${scan.signal === 'RED' ? 'bg-red-50 border border-red-100' :
