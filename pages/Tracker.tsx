...
const Tracker: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (
      !loading &&
      contextBankrollStats.startingBalance === 0 &&
      bets.length === 0
    ) {
  ...
  const totalWithdrawn = useMemo(() => {
    return bookBalances.reduce((sum, book) => sum + (book.withdrawn || 0), 0);
  }, [bookBalances]);

  const netProfit =
    contextBankrollStats.currentBalance +
    totalWithdrawn -
    contextBankrollStats.startingBalance;
  ...
  const handleImportData = async (data: {
    ...
    if (
      isCleanState ||
      confirm(`This will import ${data.bets.length} bets. Continue?`)
    ) {
  ...
  return (
    <div className="min-h-screen bg-ink-base pb-28 md:pb-20 flex flex-col font-sans text-ink-text">
      <DataManagementModal
        ...
        currentData={{
          bets,
          startingBankroll: contextBankrollStats.startingBalance,
        }}
      />
      ...
      <div className="bg-ink-base/80 backdrop-blur-md border-b border-ink-gray sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-ink-text tracking-tight">
              Performance Tracker
            </h2>
          </div>
        ...
        <section className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[minmax(100px,auto)]">
            ...
            <div className="flex justify-between items-start mb-2 relative z-10">
              <div>
                ...
                <p className="text-xs text-ink-text/60 font-mono mt-1">
                  ...
                  <span
                    className={
                      netProfit >= 0 ? "text-status-win" : "text-status-loss"
                    }
                  >
                    {netProfit >= 0 ? "+" : ""}
              ...
              <div className="hidden sm:flex gap-2">
                <div className="bg-ink-base/50 px-3 py-1 rounded-lg border border-ink-gray">
                  ...
                  <span
                    className={`text-sm font-mono font-bold ${contextBankrollStats.roi > 0 ? "text-status-win" : "text-ink-text"}`}
                  >
                    {contextBankrollStats.roi.toFixed(1)}%
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="md:col-span-1 lg:col-span-1 row-span-2 bg-ink-paper rounded-2xl border border-ink-gray shadow-lg flex flex-col overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto p-0 scrollbar-hide">
              {bets.slice(0, 10).map((bet, i) => (
                <div
                  key={bet.id}
                  className="p-3 border-b border-ink-gray/50 hover:bg-ink-base/50 transition-colors flex justify-between items-center group"
                >
          ...
          <div className="md:col-span-3 lg:col-span-4 grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatsCard
              ...
              subValue={
                advancedStats.currentStreak > 0 ? "Hot Streak" : "Cold Streak"
              }
            ...
            <StatsCard
              ...
              value={
                bets.length > 0
                  ? (
                      bets.reduce((acc, b) => acc + b.odds, 0) / bets.length
                    ).toFixed(0)
                  : "0"
              }
              subValue="Mean Price"
