...
- Do not add backend-only environment variables or server-side assumptions unless the app architecture changes. Browser-exposed configuration must use the `VITE_` prefix.
- Do not bypass or weaken the veto system when changing AI analysis behavior. Recommendations must still be rejected when hard factual support is missing.
- Do not treat AI narrative analysis as a replacement for odds, injury, weather, rest, or matchup data. Soft narratives should support conclusions, not drive them.
- Do not introduce new state stores unless `useGameContext` is no longer sufficient. Shared app state should continue to flow through the existing context pattern.
- Do not add broad styling systems, CSS modules, or component libraries without a clear reason. The project currently uses Tailwind utilities and custom `ink-*` tokens.
