...
export const BetForm: React.FC<BetFormProps> = ({
  ...
  useEffect(() => {
    if (draftBet) {
      ...
      const matchupText =
        draftBet.homeTeam && draftBet.awayTeam
      ...
      if (draftBet.sportsbook) {
        ...
        const found = Object.values(Sportsbook).find(
          (s) =>
            s
              .toLowerCase()
              .includes(draftBet.sportsbook!.toString().toLowerCase()) ||
            draftBet
              .sportsbook!.toString()
              .toLowerCase()
      ...
      if (draftBet.pickTeam) {
        ...
        const isMoneyline = market.startsWith("moneyline") || market === "ml";

        const hasLine =
          draftBet.line !== null &&
          draftBet.line !== undefined &&
          !Number.isNaN(Number(draftBet.line));
        ...
        let suffix = "";
        if (isMoneyline) {
          const lowerPick = draftBet.pickTeam.toLowerCase();
          if (!lowerPick.includes("moneyline") && !lowerPick.includes("ml")) {
            suffix = " Moneyline";
          }
        }

        const rawPick = `${draftBet.pickTeam}${suffix}${lineStr}`.trim();
  ...
  const handleScanClick = () => {
    if (!hasApiKey) {
      alert(
        "Scanner Not Available: API Key not found. Please check your settings.",
      );
      return;
    }
    ...
    try {
      ...
      const schema = {
        ...
        properties: {
          ...
          matchup: {
            type: Type.STRING,
            description: "The matchup, e.g. 'Lakers vs Celtics'",
          },
          sport: {
            type: Type.STRING,
            description: "The sport league, e.g. NFL, NBA",
          },
      ...
      if (response.text) {
        ...
        if (result.sportsbook) {
          const foundBook = SPORTSBOOKS.find((sb) =>
            sb.toLowerCase().includes(result.sportsbook.toLowerCase()),
          );
    ...
    <div className="space-y-6">
              ...
              <span>
                ...
                {calculatedPayout > 0 && (
                  <span className="opacity-80 font-mono ml-1">
                    {" "}
                    (Win {formatCurrency(calculatedPayout)})
                  </span>
                )}
              </span>
            </button>
      ...
      {isAnalyzing && (
        <div className="fixed inset-0 z-50 flex flex-col items-center justify-center bg-ink-base/80 backdrop-blur-md">
          ...
          <p className="text-white font-bold text-lg animate-pulse font-mono">
            Decoding Slip...
          </p>
        </div>
      )}
