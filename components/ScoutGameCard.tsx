...
const ScoutGameCard: React.FC<ScoutGameCardProps> = ({
  ...
  return (
    <div className="bg-ink-paper border border-ink-gray rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
      {scan && <div className={`absolute top-0 left-0 bottom-0 w-1.5 ${scan.signal === 'RED' ? 'bg-status-loss' : scan.signal === 'YELLOW' ? 'bg-amber-500' : 'bg-ink-gray'}`} />}
      <div className="flex justify-between items-center mb-2 pl-2">
        ...
        <button onClick={() => onAddToQueue(game, pinnLines)} disabled={inQueue} className={`px-2 py-1 rounded text-[10px] font-bold transition-colors ${inQueue ? 'bg-ink-base text-ink-text/40' : 'bg-teal-500/10 text-teal-400 hover:bg-teal-500/20'}`}>{inQueue ? '✓ Queue' : '+ Add'}</button>
      </div>
      <div className="mb-2 pl-2">
        ...
        <div className="grid grid-cols-[2fr_1fr_1fr_2fr] gap-1 items-center py-1 border-b border-ink-gray">
          ...
          <div className="text-center font-bold text-ink-text bg-ink-base rounded py-0.5 text-[10px] font-mono">{pinnLines?.spreadLineA || '-'}</div>
        ...
        <div className="grid grid-cols-[2fr_1fr_1fr_2fr] gap-1 items-center py-1"><div className="font-bold text-ink-text truncate text-xs">{game.home_team}</div><div className="text-center text-ink-text/40 text-[10px] font-mono">{ref?.spreadLineB || '-'}</div><div className="text-center font-bold text-ink-text bg-ink-base rounded py-0.5 text-[10px] font-mono">{pinnLines?.spreadLineB || '-'}</div></div>
      </div>
      <div className="pl-2">
        {scan ? (
