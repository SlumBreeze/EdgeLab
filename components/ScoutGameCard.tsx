...
const ScoutGameCard: React.FC<ScoutGameCardProps> = ({
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm hover:shadow-md transition-shadow relative overflow-hidden">
      ...
      <div className="flex justify-between items-center mb-2 pl-2">
        ...
        <button onClick={() => onAddToQueue(game, pinnLines)} disabled={inQueue} className={`px-2 py-1 rounded text-[10px] font-bold transition-colors ${inQueue ? 'bg-slate-100 text-slate-400' : 'bg-teal-50 text-teal-600 hover:bg-teal-100'}`}>{inQueue ? '✓ Queue' : '+ Add'}</button>
      </div>
