...
- [x] Task: Update `services/geminiService.ts` to strictly use the Gemini Pro 3 model for `analyzeGame` and `scanSlate`.
- [x] Task: Refactor the AI prompt in `geminiService.ts` to incorporate the "Professional Handicapper" persona.
- [x] Task: Modify prompt to accept "Ground Truth" context (rosters/stats) as an input parameter.
- [x] Task: Update the JSON response schema to include a `handicapper_logic` field for qualitative justification.
...
- [x] Task: Update `utils/analysisValidator.ts` to verify the presence of qualitative justification in AI outputs.
- [x] Task: Refine `calculations.ts` to allow "Playable" status for favorites up to -175 when supported by handicapper logic.
- [x] Task: Implement the "Data Quality Veto" to flag contradictions between AI reasoning and SportsDB data.
- [x] Task: Update the UI components (`QueuedGameCard.tsx`, `ScoutGameCard.tsx`) to display the new "Handicapper Logic" insights.
...
- [x] Task: Write integration tests to verify TheSportsDB caching and rate-limiting.
- [x] Task: Conduct a "Hallucination Audit" by testing known outdated scenarios (e.g., specific trades).
- [x] Task: Verify that favorites (odds < -110) are correctly flagged as Playable when justified.
